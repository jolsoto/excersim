...
def clean_ingredients(dish_name, dish_ingredients):
    ...
    """
    recipe=tuple((dish_name,set(dish_ingredients)))
    return recipe
...
def tag_special_ingredients(dish):
    ...
    """
    name, ing=dish
    spec=set(ing).intersection(SPECIAL_INGREDIENTS)
    return (name, spec)
def compile_ingredients(dishes):
    ...
    for dish in dishes:
        total_ingredients=total_ingredients.union(dish)
...
def separate_appetizers(dishes, appetizers):
    ...
    """
    fdish=set(dishes).symmetric_difference(appetizers)
    return list(fdish)
